import { AnyPgColumn, boolean, integer, pgSchema, serial, text, timestamp, uniqueIndex, varchar } from "drizzle-orm/pg-core";
...
export const userWatchHistory = userDataSchema.table('user_watch_history', {
  historyId: serial('history_id').primaryKey(),
  userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
  episodeId: integer('episode_id').notNull().references(() => episodes.episodeId, { onDelete: 'cascade' }),
  watchProgressSeconds: integer('watch_progress_seconds').default(0).notNull(),
...
export const userWatchlist = userDataSchema.table('user_watchlist', {
  ...
  status: varchar('status', { length: 20 }).default('plan_to_watch').notNull(),
}, (table) => [
    uniqueIndex('unique_user_anime').on(table.userId, table.animeId),
    uniqueIndex('idx_watchlist_user_id').on(table.userId),
]);

export const userRatings = userDataSchema.table('user_ratings', {
...
export const comments = userDataSchema.table('comments', {
  commentId: serial('comment_id').primaryKey(),
  userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
  episodeId: integer('episode_id').notNull().references(() => episodes.episodeId, { onDelete: 'cascade' }),
  parentCommentId: integer('parent_comment_id').references(() : AnyPgColumn => comments.commentId, { onDelete: 'cascade' }),
